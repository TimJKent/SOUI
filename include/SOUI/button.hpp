...
namespace SOUI
{
    ...
    public:
        ...
        {
            auto render_objects = panel_->GetRenderObjects();
            const auto &text_objects = text_->GetRenderObjects();
            render_objects.reserve(render_objects.size() + text_objects.size());
            render_objects.insert(render_objects.end(), text_objects.begin(), text_objects.end());
