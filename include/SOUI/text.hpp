...
#include "SOGLR/Rendering/Text/Font.hpp"
...
namespace SOUI
{
    ...
    {
    public:
        Text(glm::ivec2 position, glm::ivec2 size, const std::string& text, std::shared_ptr<SOGLR::Shader> shader, std::shared_ptr<SOGLR::Text::Font> font, HorizontalTextAlign h_align = HorizontalTextAlign::LEFT, VerticalTextAlign v_align = VerticalTextAlign::BOTTOM)
        : text_object_(std::make_shared<SOGLR::RenderObject>())
        , text_(text)
        , shader_(shader)
        , font_(font)
        , h_align_(h_align)
        ...
        void SetText(const std::string& new_text)
        {
            ...
            for(char c : text_)
            {
                const SOGLR::Text::Character& ch = font_->GetCharacter(c);
    ...
    private:
        ...
        std::shared_ptr<SOGLR::Text::Font> font_;
